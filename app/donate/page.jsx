import Footer from "../homeComponent/Footer"
import ProfileHeaderSect from "../me/ProfileHeaderSect"
import DoughnutCntn from "./doughnutCntn"

export async function generateMetadata() {

    return {
        title: 'Donate To Kwiva',
        description:
            "Kwiva is stuck!, we need your help to keep us going. Kindly drop any token you can spare and we'll appreciate it immensely.",
        openGraph: {
            title: 'Donate To Kwiva',
            description: 'Kwiva is stalling🥶!, we need your help to keep us going and reach more audience. Kindly donate any token you can spare and we\'ll appreciate it immensely. Thanks🙏.',
            url: "https://kwiva.online/donate",
            type: 'website',
            images: [
                {
                    url: "https://kwiva.online/donate.png"
                }
            ]
        },
        twitter: {
            title: 'Donate To Kwiva',
            creator: "@QuadVox",
            description:
                'Kwiva is stalling🥶!, we need your help to keep us going and reach more audience. Kindly drop any token you can spare and we\'ll appreciate it immensely. Thanks🙏.',
            card: 'summary_large_image',
            url: "https://kwiva.online/donate",
            images: [
                {
                    url: "https://kwiva.online/donate.png"
                }
            ]
        },
    }
}

const Page = () => {
    return (
        <div className="storyGrandCntn histories">
            <ProfileHeaderSect></ProfileHeaderSect>
            <DoughnutCntn></DoughnutCntn>
            <Footer></Footer>
        </div>
    )
}

export default Page