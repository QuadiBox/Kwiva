    ...
    return (
        <main className="quizPreviewCntn gap_var">
            ...
            <div className="ad_slot">
                <GoogleAd key={`ad-qz-gst-${number}`} adKey={`ad-qz-gst-${number}`}></GoogleAd>
            </div>

            <div className='theQuestionNavCntn'>
                {!isFirst && <Link className='prev' href={`/quizzez/gst/${questionIndex}`}>Prev</Link>}
                {!isLast ? (
                    <Link className='next' href={`/quizzez/gst/${questionIndex + 2}`}>Next</Link>
                ) : (
