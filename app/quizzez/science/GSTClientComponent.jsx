...
const GSTClientComponent = () => {
    ...
    const handleQuestionPrep = async () => {
        ...
        const quizId = `science_${Date.now()}`;
        ...
        //get "science" from localStorage
        //if not available or it's expiryTime value hass been exceeded, fetch all document that's not _meta from science collection on firestore
        //flatten questions array from science collection documents to get a giant array of objects and update the localStorage science by seting the giant array as the questions field and expiryTime set to 3 days from Date.now() 
        ...
        setLoading(true);
        window.location.href = `/quizzez/science/1`;
    }
    ...
    return (
        <>
            ...
            <div className="ad_slot">
                <GoogleAd key={`ad-qz-science-start`} adKey={`ad-qz-science-start`}></GoogleAd>
            </div>
