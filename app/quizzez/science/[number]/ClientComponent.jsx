    ...
    return (
        <main className="quizPreviewCntn gap_var">
            ...
            <div className="ad_slot">
                <GoogleAd key={`ad-qz-science-${number}`} adKey={`ad-qz-science-${number}`}></GoogleAd>
            </div>

            <div className='theQuestionNavCntn'>
                {!isFirst && <Link className='prev' href={`/quizzez/science/${questionIndex}`}>Prev</Link>}
                {!isLast ? (
                    <Link className='next' href={`/quizzez/science/${questionIndex + 2}`}>Next</Link>
                ) : (
