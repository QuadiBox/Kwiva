    ...
    return (
        <main className="quizPreviewCntn gap_var">
            ...
            <div className="ad_slot">
                <GoogleAd key={`ad-qz-arts-${number}`} adKey={`ad-qz-arts-${number}`}></GoogleAd>
            </div>

            <div className='theQuestionNavCntn'>
                {!isFirst && <Link className='prev' href={`/quizzez/arts/${questionIndex}`}>Prev</Link>}
                {!isLast ? (
                    <Link className='next' href={`/quizzez/arts/${questionIndex + 2}`}>Next</Link>
                ) : (
